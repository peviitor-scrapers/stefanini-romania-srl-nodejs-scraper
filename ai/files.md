...
| `tests/validate-stefanini-jobs.js` | **STEFANINI-specific fast validator (used by CI).** HEAD requests only, reads CIF from config. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
| `tests/unit/index.test.js` | Unit tests for index.js - parseJobsHTML, mapToJobModel, transformJobsForSOLR |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real SmartSearchOnline HTML, ANAF, and SOLR |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for SmartSearchOnline |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 16139707. Use only when you need to wipe STEFANINI jobs from SOLR entirely. |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape SmartSearchOnline → transform → upsert SOLR → generate docs/jobs.md